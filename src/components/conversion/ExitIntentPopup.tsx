...
export default function ExitIntentPopup({
  ...
  // Customize message based on cart state
  const displayOffer = hasCartItems 
    ? `Complete your booking and get ${cart.appliedCoupon ? 'an additional' : ''} 10% OFF!`
    : offer;
  
  const displayDescription = hasCartItems
    ? `You have ${getItemCount()} item${getItemCount() > 1 ? 's' : ''} in your cart worth ₹${getTotal().toLocaleString()}. Complete your booking now and get an exclusive discount!`
    : "Contact us now and get a free consultation for your project. Our experts are ready to help you with premium aluminium, glass, and netting solutions.";
        ...
        >
          ...
          <div className="text-center">
            ...
            <h2 id="exit-intent-title" className="text-2xl font-bold text-gray-900 mb-3">
              {hasCartItems ? "Don't Leave Your Cart Behind!" : "Wait! Don't Miss Out"}
            </h2>
            ...
            {/* CTA Buttons - Different based on cart state */}
            {hasCartItems ? (
              <>
                {/* View Cart Button */}
                <button
                  ...
                  aria-label="View your cart and complete booking"
                >
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-gray-600 text-white px-8 py-4 rounded-lg font-semibold hover:bg-gray-700 transition-colors w-full focus:outline-none focus:ring-2 focus:ring-gray-600 focus:ring-offset-2"
                  aria-label={`Or call us at ${BUSINESS_INFO.phone.display}`}
                >
